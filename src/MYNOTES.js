...
As mentioned in the previous lesson, React Router enables client-side routing which allows us to render different portions of our webpage using the browser's History APILinks to an external site. instead of making requests to our server for a new webpage. With client-side routing, our browser renders a new component, and our client-side JavaScript requests any data we want to display in that component. This is essential for routing in any React application, as we only have a single HTML file to serve — that's the nature of an SPA.
...
function Home() {
  ...
  Let's create a new NavBar component in the components folder to add these NavLinks to our application.
  ...
  Load up the browser again and you should see beautiful blue NavLinks that load up the desired components.
  ...
  Note that we're importing the useRouteError hook in addition to our NavBar component. The useRouteError hook allows us to interact with the error itself, including the error status and its message. You can read more about it in the useRouteError documentationLinks to an external site..
  ...
  In the coming lessons, we'll explore more of the advanced functionality provided by React Router. If you have the time, you should definitely look at the React Router docsLinks to an external site. — especially the examples — to dive deeper into React Router's many features and get a better sense of how to use it in an application.
  
  Resources
  React Router docsLinks to an external site.
